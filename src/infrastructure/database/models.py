...
from src.infrastructure.database.types import CompatJSONB, CompatUUID
...
class UserModel(Base):
    ...
    tenant_id: Mapped[str] = mapped_column(
        CompatUUID, ForeignKey("tenants.id"), nullable=False
    )
...
class MatterModel(Base):
    ...
    owner_user_id: Mapped[str | None] = mapped_column(
        CompatUUID, ForeignKey("api_users.id")
    )
    ...
    metadata_: Mapped[dict[str, Any]] = mapped_column(
        "metadata", CompatJSONB, default=dict, server_default="{}"
    )
    created_at: Mapped[datetime] = _utc_now()
    updated_at: Mapped[datetime] = _utc_now()
...
class MatterPartyModel(Base):
    ...
    matter_id: Mapped[str] = mapped_column(
        CompatUUID, ForeignKey("matters.id", ondelete="CASCADE"), nullable=False
    )
...
class EvidenceModel(Base):
    ...
    uploaded_by: Mapped[str] = mapped_column(
        CompatUUID, ForeignKey("api_users.id"), nullable=False
    )
    created_at: Mapped[datetime] = _utc_now()
...
class ChronologyEventModel(Base):
    ...
    source_id: Mapped[str | None] = mapped_column(CompatUUID)
...
class OutsideCounselModel(Base):
    ...
    scoped_access: Mapped[dict[str, Any]] = mapped_column(CompatJSONB, default=list, server_default="[]")
    engagement_terms: Mapped[dict[str, Any]] = mapped_column(
        CompatJSONB, default=dict, server_default="{}"
    )
...
class ReviewNoteModel(Base):
    ...
    related_deadline_id: Mapped[str | None] = mapped_column(
        CompatUUID, ForeignKey("procedural_deadlines.id")
    )
    related_hearing_id: Mapped[str | None] = mapped_column(
        CompatUUID, ForeignKey("hearings.id")
    )
    created_at: Mapped[datetime] = _utc_now()
...
class AIAnalysisRunModel(Base):
    ...
    input_snapshot: Mapped[dict[str, Any]] = mapped_column(CompatJSONB, nullable=False)
    output_snapshot: Mapped[dict[str, Any]] = mapped_column(CompatJSONB, nullable=False)
...
class AuditEventModel(Base):
    ...
    resource_id: Mapped[str] = mapped_column(CompatUUID, nullable=False)
    event_type: Mapped[str] = mapped_column(String(64), nullable=False)
    trace_id: Mapped[str | None] = mapped_column(String(128))
    payload: Mapped[dict[str, Any]] = mapped_column(CompatJSONB, default=dict, server_default="{}")
    created_at: Mapped[datetime] = _utc_now()
